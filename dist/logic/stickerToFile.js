...
const stickerToFile = async (ctx) => {
    const userId = ctx.from?.id;
    if (!userId)
        return ctx.reply("No userId passed");
    ...
    if (!fileId)
        return ctx.reply("FileID not passed");
    ...
    try {
        ...
        if (!fileType)
            return ctx.reply("Unable to determine file format");
        if (fileType.ext !== "gz") {
        ...
        if (format === "json") {
            await ctx.replyWithDocument(new InputFile(Buffer.from(lottieJSON), "sticker.json"), {
            ...
            await ctx.replyWithDocument(new InputFile(lottieBuffer, "sticker.lottie"), {
...
//# sourceMappingURL=stickerToFile.js.map